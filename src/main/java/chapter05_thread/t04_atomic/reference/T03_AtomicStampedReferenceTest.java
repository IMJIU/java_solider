...
/**
 * 解决问题： 解决ABA问题，通过一个引用和版本号为基础解决ABA问题，
 * 当一个对象从A变成B，再从B变成A，此时若并发中，某个线程发现对象还是A，那么此时就会对比成功
 * 为了解决这个问题，AtomicStamped使用了版本号来解决这个问题 ABA问题的模拟你参看模拟代码：
 * 
 * @author zhongyin.xy
 *
 */
public class T03_AtomicStampedReferenceTest {

	/**
	 * 常见的方法：
	 * 
	 * @see AtomicStampedReference#attemptStamp(Object, int)
	 *      用于绝对判定是否一致，返回true|false
			...
			threads[i] = new Thread() {
				public void run() {
					...
					if (ATOMIC_REFERENCE.compareAndSet(oldValue, oldValue + num, stamp, stamp + 1)) {
						System.out.println("我是线程：" + num + ",我获得了锁进行了对象修改！stamp:"+(stamp+1));
					}
				}
			};
			threads[i].start();
		...
		new Thread() {
			public void run() {
				...
				while (!ATOMIC_REFERENCE.compareAndSet(ATOMIC_REFERENCE.getReference(), "abc", stamp, stamp + 1)) {
					System.out.println("已经改为原始值！stamp:"+stamp);
