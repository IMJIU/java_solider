 ...
 *
 */
public class T01_AtomicIntegerArrayTest {
	 ...
	 *      对比修改，参数1：数组下标，参数2：原始值，参数3，修改目标值，修改成功返回true否则false
		...
		for (int i = 0; i < 100; i++) {
			...
			threads[i] = new Thread() {
				public void run() {
					...
					System.out.println("线程编号：" + threadNum + " , 对应的组为：" + (index + 1) + "，累加个数：" + result/(index+1));
				}
			};
			threads[i].start();
		}
